...
void add_event_to_list(string event) {
	...
	string prev_prop = get_property("bigoli_event_list");
	string new_prop;
	if (prev_prop == "") {
		new_prop = event;
	} else {
	...
	set_property("bigoli_event_list", new_prop);
}
void clear_event_list() {
	set_property("bigoli_event_list", "");
}
...
void compare_using_list() {
	string[int] split_map = split_string(get_property("bigoli_event_list"), ",");
